...
# N 區：API Contract & Error Boundary

> 目的：讓 HTTP 業務 API 的契約、文件與錯誤結果可被穩定整合、追蹤與維護。

---

## N-1 REST 業務 API 必須提供 OpenAPI v3.0 以上規格

**嚴重度**：Must（對外、跨服務或由其他團隊整合的 REST API）；其他內部 API 為 Should

**規則**
- 會提供 REST 業務 API 的微服務，必須有可取得的 OpenAPI v3.0 以上規格文件。
- 規格必須描述 endpoint、HTTP method、request、response、主要錯誤結果與必要的安全要求。
- review 時，只有在本次範圍可見 controller、OpenAPI 設定或 API contract 變更時，才檢查規格是否同步；不可因未看到文件而推測不存在。

**原因**
- 穩定、可讀取的契約能降低跨團隊整合與 API 演進的溝通成本。

---

## N-2 OpenAPI annotation 應補足非直覺的 API 語意

**嚴重度**：Should

**規則**
- 使用 annotation 產生 OpenAPI 時，應以 `@Tag`、`@Operation`、`@RequestBody`、`@ApiResponse`、`@Parameter` 補充名稱、用途、參數限制、回應與錯誤語意。
- 不得機械地在每個 endpoint 重複加入沒有新增資訊的 annotation。
- 對非直覺參數、非同步結果、權限限制、狀態限制與重要錯誤碼，應優先提供可讀說明。

**原因**
- annotation 的價值在於補足 contract 語意，不是增加重複標記。

---

## N-3 Spring MVC REST API 的例外處理應套用 E-2 與 J-18

**規則**
- Spring MVC REST API 的 controller 層，預設應使用 `@ControllerAdvice` 與 `@ExceptionHandler` 集中套用 E-2 的對外例外轉換；若採其他機制，必須能提供等價的一致性。
- 錯誤 JSON 的穩定 `code`、面向使用者的 `message` 與業務錯誤分類，依 J-18 審查。
- 發現直接回傳 Java exception message、stack trace、SQL、class name、內部 endpoint 或敏感資料時，finding 應引用 E-2；不要重複建立 N-3 finding。

**原因**
- 此節只補充 Spring MVC 的適用方式，避免與既有例外與業務錯誤規則重複。

---

## N-4 錯誤碼必須分類並集中定義

**嚴重度**：Must（對外 API、批次或上游系統依賴錯誤結果時）

**規則**
- 至少區分系統性錯誤、可預期的交易功能／業務錯誤，以及需要確認最終結果的交易不明。
- 錯誤碼與對外訊息必須以 enum、error catalog 或其他集中機制管理；是否使用資料庫應依既有架構決定，不得為此新增不必要的運行期依賴。
- 不得將可預期業務錯誤一律包成系統錯誤，也不得讓同一錯誤碼在不同 API 代表不同語意。

**原因**
- 穩定分類讓前端、上游、批次、客服與監控可採取正確處理方式。

---

## N-5 API 外部異動結果不明時應套用 J-6 與 J-17

**規則**
- 外部異動在 timeout、連線中斷或未知回應時，不得直接判定交易失敗或重送造成重複副作用；應依 J-6 審查冪等與重複執行風險。
- 本地交易序號、requestId／冪等鍵、外部參考號、處理狀態、查詢與人工處置方式，應依 J-17 審查可對帳性。
- 自動重試只適用於可判定為暫時性的失敗；必須有 timeout、重試上限與退避策略。發現未知副作用的寫入被直接重試時，finding 應引用 J-6，不要重複建立 N-5 finding。

**原因**
- 此節只補充 API 的適用方式，避免與既有冪等與對帳規則重複。

---

# O 區：Documentation & Comments

> 目的：保留真正有助於理解契約與決策的說明，不用重複註解製造雜訊。

---

## O-1 公開契約與非直覺領域元件應提供 Javadoc

**嚴重度**：Should

**規則**
- 非自動產生的 public API、擴充點、複雜領域元件、非直覺副作用或重要限制，應以 Javadoc 說明用途、前置條件、回傳／例外語意與必要限制。
- 不強制簡單 DTO、標準 CRUD、測試程式或語意已清楚的 private helper 撰寫 Javadoc。

**原因**
- 文件應集中在呼叫者需要知道、但無法只從名稱推得的契約。

---

## O-2 註解應解釋原因，不得重複描述程式表面行為

**嚴重度**：Should

**規則**
- 重要業務規則、特殊演算法、相容性限制、交易／補償取捨或非直覺例外處理，應說明「為什麼」採用該作法。
- 不得加入只重述程式碼名稱、控制流程或顯而易見操作的重複性註解。
- 註解若已與實作或 contract 不一致，必須同步修正或刪除。

**原因**
- 過時或重複的註解會誤導維護者，比沒有註解更危險。

---

# P 區：Observability

> 目的：讓問題可追蹤、可量測，同時避免以 log 或 metrics 洩漏敏感資料或造成不可控成本。

---

## P-1 MDC 必須建立、傳遞並清理請求共用追蹤資訊

**嚴重度**：Major

**規則**
- HTTP request 的 requestId、traceId、必要交易序號等共用診斷資訊，可在 filter 或 interceptor 放入 MDC。
- MDC 必須在 `finally` 清理；非同步 task 必須明確傳遞與清理 context，避免執行緒池重用造成跨請求污染。
- 不得把完整 HTTP header、Token、密碼、完整個資或不必要的員工識別資訊寫入 MDC。

**原因**
- MDC 可縮短排查時間，但其 ThreadLocal 特性也會造成敏感資料外洩與跨請求污染。

---

## P-2 metrics 必須使用穩定且低基數的維度

**嚴重度**：Major

**規則**
- 使用 Spring Boot Actuator、Micrometer、Prometheus、OpenTelemetry 或其他 metrics 系統時，交易狀態與錯誤碼可作為受控且有限值域的維度。
- 不得將交易序號、訂單號、員工編號、使用者 ID、email、完整 URL query 或例外訊息放入 metric label／tag。
- 新增指標時應確認名稱、單位、標籤值域與告警／儀表板用途，避免產生高基數成本。

**原因**
- 高基數 metrics 容易耗盡監控系統資源，且可能洩漏個資。

---

## P-3 已啟用 OpenTelemetry 時，日誌與 trace 必須可關聯

**嚴重度**：Should

**規則**
- 專案已啟用 OpenTelemetry 時，日誌應能輸出或關聯 `trace_id`、`span_id` 或 `traceparent`。
- 非同步工作應保留正確 parent-child trace 關係；不得只為每個步驟任意建立無關 trace。

**原因**
- 可關聯的 logs、traces 與 metrics 才能支持跨服務診斷。

---

## P-4 非同步日誌與 buffer 屬於明確配置決策

**嚴重度**：Should

**規則**
- 採用非同步日誌或 buffer 時，必須定義容量、滿載策略、遺失／阻塞取捨、flush／shutdown 行為與基本監控。
- 未見 logging 設定或效能量測證據時，不得僅因未使用非同步日誌建立 finding。

**原因**
- 非同步日誌能降低尖峰延遲，但錯誤配置可能在事故時遺失關鍵證據。

---

# Q 區：Service Orchestration

> 目的：讓跨服務流程的責任邊界、失敗狀態與最終一致性可被理解與處置。

---

## Q-1 服務編排必須有清楚的責任與領域邊界

**嚴重度**：Major

**規則**
- 編排元件必須明確說明其流程所有權、參與服務與領域邊界；不得演變為收納所有跨領域規則的通用 service。
- 領域決策應由明確擁有該規則的 domain/service 處理；編排層負責協調、狀態與技術整合時，必須能辨識其責任。
- 通路層是否負責單一通路的跨領域編排，應依既有架構與責任分工判斷，缺少證據時僅列開放問題。

**原因**
- 無主的流程決策會使領域規則分散、難測試且難以演進。

---

## Q-2 跨服務異動必須定義一致性、補償與人工處置

**嚴重度**：Must

**規則**
- 跨服務異動必須明確定義強一致性邊界；無法強一致時，必須有持久化狀態、冪等、重試、補償、對帳與必要的人工處置流程。
- 不得只以同步 API 成功回應推論整體流程已完成。

**原因**
- 分散式系統的部分失敗不可避免，必須設計可恢復與可對帳的狀態。

---

## Q-3 長時間流程必須持久化狀態並可恢復

**嚴重度**：Major

**規則**
- 短交易可使用同步服務編排；長時間、等待外部事件或需要人工介入的流程，必須保存流程狀態、逾時、重試與補償資訊。
- 是否使用特定 workflow engine 依既有平台與需求決定，不得將未採用特定工具本身視為違規。

**原因**
- 長流程不應只依賴記憶體或單次 request 生命週期保存狀態。

---

# R 區：Batch Processing

> 目的：讓批次任務具備隔離、可操作性、可恢復性與大量資料處理的安全邊界。

---

## R-1 批次框架與線上資源隔離應依現有架構明確設計

**嚴重度**：Major

**規則**
- 使用 Spring Boot 的批次服務可採 Spring Batch；不得將未使用 Spring Batch 本身視為違規，除非專案已明確指定。
- 線上交易與批次若共存，必須有獨立部署、資源配額、排程時段或其他可驗證的隔離策略，避免批次耗盡線上資源。
- 未見部署或資源設定時，應列為開放問題，不可僅由 Java 程式碼推測違規。

**原因**
- 批次負載與線上延遲目標不同，缺少隔離容易互相影響。

---

## R-2 批次必須具備受控觸發、狀態查詢與操作能力

**嚴重度**：Major

**規則**
- 批次可由 REST API、事件、外部排程工具或 Spring Scheduler 觸發，但必須明確定義觸發來源與授權邊界。
- 必須提供與實際運維需求相符的執行狀態、失敗原因與停止／取消操作方式；不強制一定以 REST API 提供。
- 觸發與控制操作應留下操作者、時間、job／execution ID 與結果紀錄。
...
## R-3 Job parameters 與批次追蹤識別必須可驗證且可關聯

**嚴重度**：Major

**規則**
- Job parameters（例如資料日、檔案位置、重跑範圍）必須驗證格式與允許範圍，並可在執行紀錄中追溯。
- Job、Step 與執行紀錄應有可關聯的 execution ID／trace ID；已啟用 OpenTelemetry 時，應維持正確 trace 關係。
- 不得把密碼、Token 或其他敏感資料放入可被日誌或執行紀錄輸出的 parameters。

**原因**
- 可追溯的輸入與識別資訊是安全重跑、排查與稽核的前提。

---

## R-4 大量資料處理必須有具體的存取與效能策略

**嚴重度**：Major

**規則**
- 大量資料可依資料量、資料庫負載與執行時間採 chunk-oriented、partitioning 或其他等價策略；不得機械要求單一模式。
- 必須避免無界一次載入、逐筆 N+1 資料庫／檔案存取，以及未設定 checkpoint 的長時間工作。
- 效能策略應以資料量、索引、I/O 模式或量測證據支持；資訊不足時列為開放問題。

**原因**
- 批次效能問題通常來自存取模式與資源競爭，不是單一框架選擇。

---

## R-5 批次必須防重複執行並支援安全恢復

**嚴重度**：Must

**規則**
- 必須設計防重複執行、冪等、重新執行／繼續執行、正常中斷與強制中斷後的狀態收斂策略。
- 錯誤必須分類並提供穩定錯誤碼與訊息，以區分可重試、可跳過與需人工處理的資料。
- 重新執行不得造成重複扣款、重複發送、重複匯入或其他不可逆副作用。

**原因**
- 批次失敗與重跑是正常運行場景，必須能安全恢復而非依賴人工猜測。
